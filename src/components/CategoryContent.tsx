...
export default function CategoryContent({ slug }: { slug: string }) {
  ...
  return (
    <>
      <h1 className="text-3xl font-extrabold text-purple-800 mb-2">{t(cat.slug)}</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {catTools.map(tool => (
          <Link key={tool.slug} href={`/tools/${tool.slug}`} className="card group">
            <h2 className="font-bold text-purple-800 group-hover:text-purple-500 transition mb-2">
              {t(tool.slug)}
            </h2>
